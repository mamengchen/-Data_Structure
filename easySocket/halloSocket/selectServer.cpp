...
int procss_function(SOCKET cliSock) {
	...
	case LOGIN:
		...
		sprintf(usertext.text, "用户 %s 完成工作", user.username);
		break;
	case LOGOUT:
		printf("收到命令: LOGOUT || 命令长度: %d \n", header.datalength);
		recv(cliSock, (char*)&user + sizeof(DataHeader), sizeof(UserInfor) - sizeof(DataHeader), 0);
		sprintf(usertext.text, "用户 %s 删除工作", user.username);
		break;
	default:
...
int main() {
	...
	while (true) {
		...
		if (FD_ISSET(sock, &readfd)) {
			...
			for (int i = 0; i < mysocker.size(); i++) {
				NewUserJoin newuser = {};
				newuser.restion = (int)cliSock;
				send(mysocker[i], (char*)&newuser, sizeof(NewUserJoin), 0);
			}
